...
function appendPaymentTable(curPayment) {
  ...
  appendDeleteBtnToPayment(newTr, 'payment' + paymentId);  // Append delete button to each payment row
...
function appendDeleteBtnToPayment(tr, paymentId) {
  let deleteBtn = document.createElement('button');
  deleteBtn.innerText = 'X';
  deleteBtn.onclick = function() {
    deletePayment(paymentId);  // Function to handle the deletion of the payment
  };
...
// Function to handle the deletion of the payment
function deletePayment(paymentId) {
  delete allPayments[paymentId];  // Delete the payment from the allPayments object
  
  let tr = document.getElementById(paymentId);
  if (tr) {
    tr.remove();  // Remove the payment row from the table
  }
