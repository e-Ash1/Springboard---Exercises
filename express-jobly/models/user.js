  ...
  class User {
    ...
    static async filterByJobCriteria({ title, minSalary, hasEquity }) {
      let query = `SELECT id, title, salary, equity FROM jobs`;
      let whereConditions = [];
      let queryValues = [];

      if (title) {
          whereConditions.push(`LOWER(title) LIKE LOWER($${whereConditions.length + 1})`);
  ...
  /** Apply for a job
   *
   * - username: the applying user's username
   * - jobId: the job ID being applied to
   *
  ...
  static async applyForJob(username, jobId) {
    ...
    // Attempts to create the application
    try {
      const res = await db.query(
        `INSERT INTO applications (username, job_id) VALUES ($1, $2) RETURNING job_id AS "jobId"`,
        [username, jobId]
      );
      return { applied: res.rows[0].jobId };
    } catch (error) {
      if (error.code === "23505") { // Duplicate entry code
